<?php
namespace ElementorMovieWidgets\Widgets;
 ...
 * Class Portfolio
 */
class MoviePortfolio extends Base_Widget {
	...
	private $movieArray = [];
	private $movieMetaKey = 'movie-statuses';
	...
	public function get_title() {
		return __( 'Movie Portfolio', 'elementor-pro' );
	}
	...
	public function render() {
		...
		if (is_user_logged_in()) {
			$this->movieArray = (get_user_meta(get_current_user_id(), $this->movieMetaKey, true)) ? get_user_meta(get_current_user_id(), $this->movieMetaKey, true) : "" ;
		}
		...
		while ( $wp_query->have_posts() ) {
			...
			$this->render_post($this->movieArray);
		}
	...
	protected function render_post_header($seen) {
		global $post;
		...
		<article <?php post_class( $classes ); ?>>
			<a class="elementor-post__thumbnail__link<?php echo $seen ? " seen" : ""; ?>" href="<?php echo get_permalink(); ?>">
		<?php
	}
	...
	protected function check_seen_status($movieArray) {
		global $post;
		$movieIndex = array_search($post->ID, $movieArray, false);
		switch ($movieArray[$movieIndex+1]) {
			case 1:
				return true;
				break;
			case 0:
	...
	protected function render_post($movieArray) {
		...
		$this->render_post_header($seenBool);
